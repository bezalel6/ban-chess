...
const customBadWords = [
  ...
  'lichess',
  'chesscom',
  'chess.com', // Brand impersonation
  'stockfish',
  'engine',
  'bot', // Bot impersonation
];
...
export function containsProfanity(username: string): boolean {
  ...
  if (RESERVED_USERNAMES.has(lower)) {
    console.log(
      `[Security] Blocked reserved username attempt: ${username.substring(0, 3)}***`
    );
    return true;
  }
  ...
  if (
    ...
    console.log(
      `[Security] Blocked admin impersonation attempt: ${username.substring(0, 3)}***`
    );
    return true;
  }
  ...
  if (
    ...
    console.log(
      `[Security] Blocked system impersonation attempt: ${username.substring(0, 3)}***`
    );
    return true;
  }
  ...
  if (filter.isProfane(username)) {
    console.log(
      `[Security] Blocked inappropriate username: ${username.substring(0, 3)}***`
    );
    return true;
  }
