import { pgTable, uuid, varchar, timestamp, integer, text, jsonb, boolean, index, serial, real } from 'drizzle-orm/pg-core';
...
export const games = pgTable('games', {
  ...
  pgn: text('pgn'),
  fenInitial: varchar('fen_initial', { length: 150 }).default('rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1 b:ban'),
  fenFinal: varchar('fen_final', { length: 100 }),
  result: varchar('result', { length: 10 }), // '1-0', '0-1', '1/2-1/2', '*'
  timeControl: jsonb('time_control').$type<{ initial: number; increment: number }>(),
  ...
  finalPosition: jsonb('final_position').$type<Record<string, string | number | boolean>>(), // Store final board state
}, (table) => ({
...
export const moves = pgTable('moves', {
  ...
  gameMovesIdx: index('idx_moves_game_moves').on(table.gameId, table.moveNumber),
}));

// Move buffer table - temporary storage for batching
export const moveBuffer = pgTable('move_buffer', {
  id: serial('id').primaryKey(),
...
export const systemSettings = pgTable('system_settings', {
  ...
  lastModifiedIdx: index('idx_system_settings_modified').on(table.lastModified),
}));