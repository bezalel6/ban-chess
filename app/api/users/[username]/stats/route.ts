...
export async function GET(
  ...
  try {
    ...
    if (username.toLowerCase().startsWith('guest')) {
      return NextResponse.json({ error: 'Guest accounts do not have statistics' }, { status: 404 });
    }
    ...
    const winRate = totalGames > 0 ? Math.round((userData.gamesWon / totalGames) * 100) : 0;
