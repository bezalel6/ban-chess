...
export default function AnalysisPage() {
  ...
  const currentGameState = useMemo((): SimpleGameState => {
    ...
    if (nextAction === 'ban') {
      ...
      legalActions = legalBans.map(move => `${move.from}${move.to}${move.promotion || ''}`);
    } else {
      const legalMoves = currentGame.legalMoves();
      legalActions = legalMoves.map(move => `${move.from}${move.to}${move.promotion || ''}`);
    }
  ...
  const applyAction = useCallback((game: BanChess, action: { move?: Move; ban?: Ban }) => {
    try {
      if (action.move) {
        const convertedMove = convertMoveForBanChess(action.move);
        // Use the game.play method with the validated move object
        return (game as { play: (action: unknown) => { success: boolean; error?: string } }).play({ move: convertedMove });
      } else if (action.ban) {
        const convertedBan = convertBanForBanChess(action.ban);
        // Use the game.play method with the validated ban object
        return (game as { play: (action: unknown) => { success: boolean; error?: string } }).play({ ban: convertedBan });
      }
  ...
  const handleMove = useCallback((move: Move) => {
    ...
    if (result.success) {
      try {
        const serialized = (BanChess as { serializeAction: (action: unknown) => string }).serializeAction({ move: convertMoveForBanChess(move) });
  ...
  const handleBan = useCallback((ban: Ban) => {
    ...
    if (result.success) {
      try {
        const serialized = (BanChess as { serializeAction: (action: unknown) => string }).serializeAction({ ban: convertBanForBanChess(ban) });
