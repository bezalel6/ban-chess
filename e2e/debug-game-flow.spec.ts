...
test.describe('Solo Game Flow Debug Tests', () => {
  test.beforeEach(async ({ page }) => {
    ...
    if (await usernameInput.isVisible()) {
      ...
      const successOrError = await Promise.race([
        page
          .waitForSelector('button:has-text("Play Solo"):not([disabled])', {
            timeout: 5000,
            state: 'visible',
          })
          .then(() => 'success'),
        page
          .waitForSelector('text=/Username is already taken/i', {
            timeout: 1000,
            state: 'visible',
          })
          .then(() => 'taken'),
  ...
  test('debug complete solo game flow without refresh', async ({ page }) => {
    ...
    await page.waitForSelector('.cg-board, .cg-wrap, .chess-board-container', {
      timeout: 15000,
    });
    ...
    console.log(
      `✅ Legal actions available: ${initialState.legalActions?.length} actions`
    );
    ...
    while (moveCount + banCount < maxActions && !initialState.isGameOver) {
      ...
      const currentState = await captureGameState(
        page,
        `Action-${actionNumber}`
      );
      ...
      console.log(
        `   Legal Actions: ${currentState.legalActions?.length || 0}`
      );
      ...
      if (
        ...
        currentState.legalActions.length === 0
      ) {
      ...
      const newState = await captureGameState(
        page,
      ...
      if (newState.status === currentState.status) {
        ...
        console.log(
          `   ✅ Status changed: "${currentState.status}" → "${newState.status}"`
        );
      }
    }
            ...
            for (const dest of destSquares) {
              ...
              if (style) {
                ...
                if (match) {
                  ...
                  if (
                    Math.abs(dx - (x - wrapRect.left)) < 10 &&
