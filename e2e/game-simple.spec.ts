...
test.describe('Game Queue - Simple', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page
      .locator('.fixed.inset-0.bg-black\\/90')
  ...
  test('should handle WebSocket connection states', async ({ page }) => {
    ...
    const joinQueueButton = page.getByRole('button', {
      name: /join queue|connecting|login required/i,
    });

    // Should eventually show "Join Queue" when connected
    await expect(joinQueueButton).toContainText('Join Queue', {
      timeout: 10000,
    });
  });
});
