...
import {
  ...
  isPieceSelected,
  getBoardInfo,
} from './utils/chess-board-helpers';
...
test.describe('Full Solo Game Flow Test', () => {
  test.beforeEach(async ({ page }) => {
    ...
    if (currentUrl.includes('/auth/signin')) {
      ...
      const usernameInput = page
        .locator(
          'input[type="text"], input[name="username"], input[placeholder*="username"]'
        )
        .first();
      ...
      const submitButton = page.locator(
        'button[type="submit"], button:has-text("Sign in"), button:has-text("Play"), button:has-text("Continue")'
      );
    ...
    await page.waitForSelector(
      'button:has-text("Play Solo (Practice)"), button:has-text("Play Solo")',
      { timeout: 15000 }
    );
  });

  test('complete solo game sequence: white ban -> white move -> black ban -> black move', async ({
    page,
  }) => {
    ...
    await page.screenshot({
      path: 'test-results/full-game-step-1-initial.png',
    });
    ...
    while (
      ...
      !currentStatus.toLowerCase().includes('game over')
    ) {
      actionCount++;
      console.log(
      ...
      if (currentStatus.toLowerCase().includes('ban')) {
        // Ban phase
        const whoBans = currentStatus.toLowerCase().includes('white')
          ? 'white'
          : 'black';
        ...
        // Move phase
        const whoMoves = currentStatus.toLowerCase().includes('white')
          ? 'white'
          : 'black';
  ...
  test('verify board orientation matches current player in solo game', async ({
    page,
  }) => {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      const currentPlayer = status.toLowerCase().includes('white')
        ? 'white'
        : 'black';
      ...
      console.log(
        `Check ${i + 1}: ${currentPlayer} to play, board shows ${orientation} perspective, matches: ${matches}`
      );
...
async function getGameStatus(page: Page): Promise<string> {
  const statusElement = page.locator(
    '.text-lg.font-semibold, .game-status, [data-testid="game-status"]'
  );
