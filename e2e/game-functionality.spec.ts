...
test.describe('Game Functionality with Browser Automation', () => {
  ...
  test('complete solo game flow with state verification', async ({ page }) => {
    ...
    await page.waitForLoadState('networkidle');
    await page.waitForSelector('.cg-wrap, .chess-board-container', { timeout: 15000 });
    
    console.log('✅ Solo game started successfully');
    await page.screenshot({ path: 'test-results/game-func-step-2-game-started.png' });
    ...
    while (actionCount < maxActions && !currentStatus.toLowerCase().includes('checkmate')) {
      actionCount++;
      ...
      if (currentStatus.toLowerCase().includes('ban')) {
        // Handle ban phase
        const color = currentStatus.toLowerCase().includes('white') ? 'white' : 'black';
    ...
    console.log(`✅ Game flow completed successfully! ${bans} bans, ${moves} moves, ${actionCount} total actions`);
  ...
  test('verify no refresh needed for player actions', async ({ page }) => {
    ...
    const initialNavigationId = await page.evaluate(() => performance.navigation.type);
    ...
    for (let i = 0; i < 4; i++) {
      ...
      if (gameState.status.toLowerCase().includes('ban')) {
        await executeBanAction(page, gameState.status.toLowerCase().includes('white') ? 'white' : 'black');
      } else {
        await executeMoveAction(page, gameState.status.toLowerCase().includes('white') ? 'white' : 'black');
      }
      ...
      const currentNavigationId = await page.evaluate(() => performance.navigation.type);
...
async function handleAuthentication(page: Page): Promise<void> {
  ...
  if (await signInLink.isVisible({ timeout: 2000 }).catch(() => false)) {
    ...
    // Submit form
    const submitButton = page.locator('button[type="submit"], button').filter({ hasText: /Sign in|Play|Continue/ }).first();
  ...
  await page.waitForSelector('button').filter({ hasText: /Play Solo/ }).first().waitFor({ timeout: 15000 });
}

async function captureGameState(page: Page): Promise<{ status: string; timestamp: number }> {
  const statusElement = page.locator('.text-lg.font-semibold, .game-status, [data-testid="game-status"], .status').first();
  const status = await statusElement.textContent({ timeout: 5000 }).catch(() => 'Unknown Status');
...
async function executeBanAction(page: Page, color: 'white' | 'black'): Promise<boolean> {
...
async function executeMoveAction(page: Page, color: 'white' | 'black'): Promise<boolean> {
  ...
  for (const move of moves) {
    try {
      ...
      const isSelected = await page.locator('.cg-wrap square.selected, .cg-wrap square.move-dest').count() > 0;
...
async function clickBoardSquare(page: Page, file: number, rank: number): Promise<void> {
