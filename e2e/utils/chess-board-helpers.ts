...
export async function getBoardInfo(page: Page): Promise<BoardInfo> {
  return await page.evaluate(() => {
    ...
    if (!board) {
      ...
      if (!container) {
        throw new Error(
          'Chess board not found. Tried .cg-wrap and .chess-board-container'
        );
      }
...
export async function isPieceSelected(page: Page): Promise<boolean> {
  return await page.evaluate(() => {
    ...
    const banSquares = wrap.querySelectorAll(
      'square.ban-dest, square.premove-dest'
    );

    return (
      !!selectedSquare || moveDestSquares.length > 0 || banSquares.length > 0
    );
  });
}
 ...
 */
async function isValidMoveDestination(
  page: Page,
  square: string,
  boardInfo: BoardInfo
): Promise<boolean> {
      ...
      for (const destSquare of moveDestSquares) {
        ...
        if (style) {
          ...
          if (match) {
            ...
            if (
              Math.abs(squareX - (x - wrap.getBoundingClientRect().left)) < 5 &&
...
export async function makeMove(
  ...
  if (!isSelected) {
    ...
    if (!isSelectedRetry) {
      console.log(
        `ERROR: Cannot select piece at ${from}. Move may be invalid or banned.`
      );
      return;
    }
  }
  ...
  if (!isValidDest) {
    console.log(
      `ERROR: ${to} is not a valid move destination for piece at ${from}`
    );
...
export async function isGameOver(page: Page): Promise<boolean> {
  const gameOverTexts = ['checkmate', 'stalemate', 'draw', 'wins', 'game over'];
