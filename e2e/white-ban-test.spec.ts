...
test.describe('Solo Game Flow - Direct Testing', () => {
  test('direct solo game flow verification', async ({ page }) => {
    ...
    await page.screenshot({
      path: 'test-results/direct-step-1-loaded.png',
      fullPage: true,
    });
    ...
    console.log(
      `Found ${allButtons.length} buttons and ${allLinks.length} links`
    );
    ...
    if (foundElement) {
      ...
      if (newUrl.includes('/game/') || newUrl.includes('/play/')) {
        ...
        if (boardFound) {
          await page.screenshot({
            path: 'test-results/direct-step-2-game-board.png',
            fullPage: true,
          });
          ...
          console.log(
            '✅ Successfully verified solo game can be started without refresh!'
          );
        } else {
      ...
      if (
        ...
        if (hasSignIn) {
          ...
          await page.waitForTimeout(2000);
          await page.screenshot({
            path: 'test-results/direct-step-auth.png',
            fullPage: true,
          });
        }
      }
    ...
    // Final screenshot
    await page.screenshot({
      path: 'test-results/direct-final-state.png',
      fullPage: true,
    });
