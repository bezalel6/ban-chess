...
test.describe('Multiplayer Game Flow', () => {
  ...
  test('two players can match and see each other', async ({ browser }) => {
    ...
    try {
      ...
      const p1InQueue = await player1.page.isVisible(
        'h2:has-text("Finding Opponent")'
      );
      const p2InQueue = await player2.page.isVisible(
        'h2:has-text("Finding Opponent")'
      );
      ...
      const [p1Board, p2Board] = await Promise.all([
        player1.page
          .waitForSelector('.cg-wrap, [class*="chessboard"]', {
            state: 'visible',
            timeout: 10000,
          })
          .catch(() => null),
  ...
  test('players can exchange moves', async ({ browser }) => {
    ...
    try {
      ...
      console.log(
        '🎯 Game started, testing move exchange using board clicks...'
      );

      // Import the helper functions
      const { makeMove, waitForTurn } = await import(
        './utils/chess-board-helpers'
      );
      ...
      const p1HasWhiteTurn = await player1.page
        .locator('text=/White to move/i')
        .isVisible()
        .catch(() => false);
      const p2HasWhiteTurn = await player2.page
        .locator('text=/White to move/i')
        .isVisible()
        .catch(() => false);
      ...
      console.log(
        `White: ${whitePlayer.username}, Black: ${blackPlayer.username}`
      );
      ...
      console.log(
        '✅ Players successfully exchanged moves using board clicks!'
      );
    } finally {
      await player1.context.close();
      await player2.context.close();
  ...
  test('disconnection is handled gracefully', async ({ browser }) => {
    ...
    try {
      ...
      for (const indicator of disconnectIndicators) {
        if (
          await player2.page
            .isVisible(indicator, { timeout: 10000 })
            .catch(() => false)
        ) {
  ...
  test('resign button works during game', async ({ browser }) => {
    ...
    try {
      ...
      if (await resignButton.isVisible()) {
        ...
        const confirmButton = player1.page.locator(
        ...
        await Promise.all([
          player1.page.waitForSelector('text=/game over|resigned|wins/i', {
            timeout: 10000,
          }),
          player2.page.waitForSelector('text=/game over|resigned|wins/i', {
            timeout: 10000,
          }),
        ]);
        ...
        console.log(
          '⚠️ Resign button not visible - feature may not be implemented'
        );
      }
    } finally {
      await player1.context.close();
  ...
  test('ban system affects both players', async ({ browser }) => {
    ...
    try {
      ...
      const banInput = player1.page.locator(
        'input[placeholder*="ban" i], input[placeholder*="word" i]'
      );
      if (await banInput.isVisible({ timeout: 5000 })) {
        ...
        console.log(
          '⚠️ Ban system UI not found - feature may not be implemented'
        );
      }
    } finally {
      await player1.context.close();
