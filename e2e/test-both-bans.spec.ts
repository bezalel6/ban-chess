...
test.describe('Comprehensive Solo Game Flow Tests', () => {
  ...
  test('complete solo game: white ban -> white move -> black ban -> black move -> continue', async ({
    page,
  }) => {
    ...
    await page.screenshot({
      path: 'test-results/solo-step-1-authenticated.png',
      fullPage: true,
    });
    ...
    await page.screenshot({
      path: 'test-results/solo-step-2-game-started.png',
      fullPage: true,
    });
    ...
    for (let i = 0; i < sequence.length; i++) {
      ...
      if (currentState.status.toLowerCase().includes('ban')) {
        const color = currentState.status.toLowerCase().includes('white')
          ? 'white'
          : 'black';
      ...
      // Analyze new state
      currentState = await analyzeGameState(
        page,
        `After Action ${actionNumber}`
      );
    ...
    const bans = executedActions.filter(action =>
      action.includes('ban')
    ).length;
    const moves = executedActions.filter(action =>
    ...
    const whiteActions = executedActions.filter(action =>
      action.includes('white')
    ).length;
    const blackActions = executedActions.filter(action =>
      action.includes('black')
    ).length;
    ...
    console.log(
      `   🎨 Color distribution: ${whiteActions} white, ${blackActions} black`
    );

    await page.screenshot({
      path: 'test-results/solo-final-state.png',
      fullPage: true,
    });
  });
  ...
  test('verify board orientation matches current player', async ({ page }) => {
    ...
    for (let i = 0; i < 6; i++) {
      ...
      const currentPlayer = state.status.toLowerCase().includes('white')
        ? 'white'
        : 'black';
      ...
      console.log(
        `Check ${i + 1}: ${currentPlayer} to act, board shows ${boardOrientation} perspective, matches: ${matches}`
      );
...
async function handleAuthenticationFlow(page: Page): Promise<void> {
  ...
  const playSoloButton = page
    ...
    .first();
  const isAuthenticated = await playSoloButton
  ...
  const signInLink = page
    ...
    .first();
  const needsAuth = await signInLink
    .isVisible({ timeout: 2000 })
    .catch(() => false);
  ...
  if (needsAuth) {
    ...
    const usernameInput = page
      .locator('input[type="text"], input[name="username"], input')
      .first();
    const hasInput = await usernameInput
      .isVisible({ timeout: 2000 })
      .catch(() => false);
    ...
    if (hasInput) {
      ...
      const submitButton = page
        ...
        .filter({ hasText: /Sign in|Login|Continue|Submit/ })
        .first();
...
async function startSoloGame(page: Page): Promise<boolean> {
  try {
    ...
    const playSoloButton = page
      ...
      .filter({ hasText: /Play Solo/ })
      .first();
    const isVisible = await playSoloButton
    ...
    const boardVisible = await page
      .waitForSelector('.cg-wrap, .chess-board-container', { timeout: 15000 })
      .then(() => true)
      .catch(() => false);
...
async function analyzeGameState(
  ...
  const isGameOver =
    status.toLowerCase().includes('checkmate') ||
    status.toLowerCase().includes('stalemate') ||
    status.toLowerCase().includes('draw');

  console.log(
    `   📊 ${label}: "${status}", Legal actions: ${hasLegalActions}, Game over: ${isGameOver}`
  );
...
async function performBanAction(
  page: Page,
  color: 'white' | 'black'
): Promise<boolean> {
...
async function performMoveAction(
  page: Page,
  color: 'white' | 'black'
): Promise<boolean> {
  ...
  for (const move of moves) {
    try {
      ...
      if (isSelected) {
        ...
        console.log(
          `      ⚠️ Piece not selected at ${move.from}, trying next move...`
        );
      }
    } catch (error) {
