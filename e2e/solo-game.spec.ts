...
test.describe('Solo Game Mode', () => {
  ...
  test('can create and join a solo game', async ({ page }) => {
    ...
    await page.screenshot({
      path: 'test-results/solo-game-board-state.png',
      fullPage: true,
    });
    ...
    for (const selector of gameStatusOptions) {
      ...
      if (await locator.isVisible().catch(() => false)) {
        foundStatus = true;
        console.log(
          `Found game indicator: ${typeof selector === 'string' ? selector : 'SoloPlayer username'}`
        );
        break;
      }
    }
  ...
  test('can play moves as both colors', async ({ page }) => {
    ...
    const usernameTexts = await Promise.all(
      usernames.map(u => u.textContent())
    );
    ...
    console.log(
      '✅ Solo game is working - both colors controlled by same player!'
    );
  });
  ...
  test('ban system works in solo mode', async ({ page }) => {
    ...
    await page.waitForSelector('text=/(White to move|Playing as White)/i', {
      timeout: 10000,
    });
    await makeMove(page, 'e2', 'e4');
  ...
  test('can complete a full game in solo mode', async ({ page }) => {
    ...
    console.log(
      `✅ Game completed ${moveCount} moves. Status: ${isActive ? 'Still playing' : 'Game ended'}`
    );
  });
});
