...
export default function WebSocketTester() {
  ...
  useEffect(() => {
    ...
    // Set default based on environment and page protocol
    if (host === 'localhost') {
      // For localhost, match the page protocol to avoid mixed content
      if (isHttps) {
        setWsUrl('wss://localhost:3001');
        setUseSecure(true);
      } else {
      ...
      // Other hosts - match page protocol
      const protocol = isHttps ? 'wss:' : 'ws:';
      setWsUrl(`${protocol}//${host}:3001`);
      setUseSecure(isHttps);
    }
  }, []);
  ...
  const runComprehensiveTests = async () => {
    ...
    const tests: ConnectionTest[] = [
      {
        url: 'wss://ws-chess.rndev.site',
        label: 'Production WebSocket (WSS)',
        status: 'pending',
      },
      {
        url: 'wss://ws.chess.rndev.site',
        label: 'Legacy WebSocket (WSS)',
        status: 'pending',
      },
      {
        url: 'ws://localhost:3001',
        label: 'Local Development (WS)',
        status: 'pending',
      },
      {
        url: 'ws://ws-chess.rndev.site',
        label: 'Production Non-Secure (WS)',
        status: 'pending',
      },
    ];
      ...
      <div className='max-w-6xl mx-auto space-y-6'>
        <div className='bg-gray-900 rounded-lg p-6 border border-gray-800'>
          ...
          <p className='text-gray-400'>
            Test WebSocket connections and debug communication issues
          </p>
        </div>
