      ...
      <div className="flex flex-col">
        ...
        <div className="grid grid-cols-2 gap-2 mb-3">
          <div className="bg-background-tertiary rounded-md p-2">
            ...
            <div className="text-sm font-semibold text-foreground">{formatTimeControl()}</div>
          </div>
          
          <div className="bg-background-tertiary rounded-md p-2">
            <div className="text-xs text-foreground-muted uppercase">Mode</div>
            <div className="text-sm font-semibold text-foreground">{gameMode}</div>
          </div>
        </div>
        
        {gameState.playerColor && (
          <div className="bg-background-tertiary rounded-md p-2 mb-3">
            <div className="text-xs text-foreground-muted uppercase">Playing as</div>
            <div className="text-sm font-semibold text-foreground capitalize">{gameState.playerColor}</div>
          </div>
        )}
        ...
        {gameState.gameOver ? (
          ...
          <>
            ...
            <div className={`rounded-md p-2 mb-3 text-center ${
              isPlayerTurn 
                ? 'bg-primary/20 border border-primary/30' 
                : 'bg-background-tertiary'
            }`}>
              <div className={`text-sm font-semibold ${
                isPlayerTurn ? 'text-primary' : 'text-foreground-muted'
              }`}>
                {nextAction === 'ban' 
                  ? (isPlayerTurn ? '🎯 Select a move to ban' : '⏳ Opponent banning...')
