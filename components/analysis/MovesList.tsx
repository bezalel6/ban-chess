...
export default function MovesList({
  ...
  const fullMoves = useMemo(() => {
    ...
    if (
      currentMove.white.ban ||
      currentMove.white.move ||
      currentMove.black.ban ||
      currentMove.black.move
    ) {
      result.push(currentMove);
    }
    ...
    <div className='w-full overflow-hidden'>
              ...
              >
                ...
                <div className='flex-1 flex items-center gap-0.5'>
                  ...
                  {fullMove.white.ban && (
                    ...
                    >
                      <span className='text-[8px] text-destructive-400 mr-0.5'>
                        ×
                      </span>
      ...
      {moves.length > 0 && (
        <div className='mt-4 pt-4 border-t border-border text-sm text-foreground-muted'>
          Total actions: {moves.length} (
          {moves.filter(m => m.startsWith('b:')).length} bans,{' '}
          {moves.filter(m => m.startsWith('m:')).length} moves)
        </div>
      )}
    </div>
  );
