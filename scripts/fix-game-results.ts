...
async function fixGameResults() {
  ...
  try {
    ...
    for (const game of games) {
      ...
      if (resultLower.includes('white wins') || resultLower.includes('white won')) {
        standardResult = '1-0';
      } else if (resultLower.includes('black wins') || resultLower.includes('black won')) {
        standardResult = '0-1';
      } else if (resultLower.includes('draw') || resultLower.includes('stalemate')) {
      ...
      if (resultReason === 'unknown' || !resultReason) {
        ...
        } else if (resultLower.includes('resignation') || resultLower.includes('resigned')) {
          resultReason = 'resignation';
        } else if (resultLower.includes('time') || resultLower.includes('timeout')) {
      ...
      try {
        ...
        console.log(`  ✅ Fixed: result="${standardResult}", reason="${resultReason}"\n`);
        fixed++;
      } catch (error) {
        console.log(`  ❌ Error updating: ${error instanceof Error ? error.message : String(error)}\n`);
      }
    }
